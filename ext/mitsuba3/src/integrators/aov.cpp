...
public:
    MI_IMPORT_BASE(SamplingIntegrator, m_use_nested)
    ...
    AOVIntegrator(const Properties &props) : Base(props) {
        ...
        for (const std::string &token: tokens) {
            ...
            } else if (item[1] == "alpha") {
                m_aov_types.push_back(Type::Alpha);
                m_aov_names.push_back(item[0] + ".T");
            } else if (item[1] == "emission") {
                m_aov_types.push_back(Type::Emission);
        ...
        for (size_t i = 0; i < m_aov_types.size(); ++i) {
            switch (m_aov_types[i]) {
                ...
                case Type::Emission: {
                    ...
                    auto emission = Spectrum(0.f);

                    auto emitter = si.emitter(scene);

                    emission = emitter->eval(si, active);
                ...
                case Type::Wi: {
                    // TODO: arbitrary s for wi computation
                    Vector3f arbitrary_s = Vector3f(0.577f, 0.577f, 0.577f);
                    auto world_wi = si.sh_frame.to_world(si.wi);
                    si.initialize_sh_frame_arbitrary(arbitrary_s);
                    auto local_wi = si.sh_frame.to_local(world_wi);
                    *aovs++ = ((local_wi.x()+1.0f)/2.0f);
                    *aovs++ = ((local_wi.y()+1.0f)/2.0f);
                    *aovs++ = abs(local_wi.z());
                    }
                    break;
                ...
                case Type::Alpha:
                    *aovs++ = si.bsdf()->eval_alpha(si, active);
                    break;
                ...
                case Type::IntegratorRGBA: {
                        ...
                        if (m_use_nested) {
                            ...
                            // Nested rendering skipped, replace with zeros
                            *aovs++ = Float(0.f); 
                            *aovs++ = Float(0.f); 
                            *aovs++ = Float(0.f);
                            *aovs++ = Float(0.f);
