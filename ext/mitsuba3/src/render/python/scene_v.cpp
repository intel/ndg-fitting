...
MI_PY_EXPORT(Scene) {
    ...
    MI_PY_CLASS(Scene, Object)
        ...
        .def("bsdfs", py::overload_cast<>(&Scene::bsdfs), D(Scene, bsdfs))
        .def("shapes_dr", &Scene::shapes_dr, D(Scene, shapes_dr))
        .def_method(Scene, total_parameters)
        ...
        .def("shapegroups", [](const Scene &scene) {
            py::list result;
            for (const Shape *s : scene.shapegroups()) {
                    result.append(py::cast(s));
            }
            return result;
        }, D(Scene, shapegroups))
