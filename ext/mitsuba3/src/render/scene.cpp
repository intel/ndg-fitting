...
MI_VARIANT int Scene<Float, Spectrum>::total_parameters() const {

    int total_parameters = 0;

    for (auto shape : m_shapes)
    {
        if (shape->id().find("var") != std::string::npos) total_parameters += shape->num_parameters();
    }

    for (auto shapegroup : m_shapegroups)
    {
        if (shapegroup->id().find("var") != std::string::npos) total_parameters += shapegroup->num_parameters();
    }

    for (auto emitter : m_emitters)
    {
        if (emitter->id().find("var") != std::string::npos) total_parameters += emitter->num_parameters();
    }

    for (auto bsdf : m_bsdfs)
    {
        if (bsdf->id().find("var") != std::string::npos) total_parameters += bsdf->num_parameters();
    }
