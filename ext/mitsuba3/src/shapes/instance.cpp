...
public:
    MI_IMPORT_BASE(Shape, m_id, m_to_world, m_to_object, mark_dirty, m_emitter)
    ...
    Instance(const Properties &props) : Base(props) {
        for (auto &kv : props.objects()) {
            ...
            if (shape && shape->is_shapegroup()) {
                ...
                if (m_shapegroup->has_emitter())
                {
                    m_emitter = m_shapegroup->get_emitter()->emitter();
    ...
    PositionSample3f sample_position(Float time, const Point2f& sample, Mask active = true) const
    {
        PositionSample3f ps = m_shapegroup->get_emitter()->sample_position(time, sample, active);
    ...
    Float pdf_position(const PositionSample3f& ps, Mask active = true) const {
        PositionSample3f ps_new = ps;
        ps_new.p = m_to_object.value().transform_affine(ps_new.p);
        ps_new.n = m_to_object.value().transform_affine(ps_new.n);

        return m_shapegroup->get_emitter()->pdf_position(ps_new, active);
    }
    ...
    virtual void optix_prepare_geometry() override {
        m_shapegroup->optix_prepare_geometry();        
    }
#endif
